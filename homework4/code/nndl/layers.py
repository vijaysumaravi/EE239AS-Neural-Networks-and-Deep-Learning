...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    running_mean = momentum * running_mean + (1 - momentum) * sample_mean
    running_var = momentum * running_var + (1 - momentum) * sample_var
    xVar = (x-sample_mean)/(np.sqrt(sample_var + eps))
    out = gamma * xVar + beta
    cache = (x, xVar, sample_mean, sample_var, gamma, beta, bn_param)
    ...
    x_var = (x-running_mean)/(np.sqrt(running_var + eps))
    out = gamma * x_var + beta
...
def batchnorm_backward(dout, cache):
  ...
  x, xVar, sample_mean, sample_var, gamma, beta, bn_param = cache
  eps = bn_param.get('eps', 1e-5)
  m = x.shape[0]
  # ================================================================ #
  # YOUR CODE HERE:
  ...
  dgamma = np.sum(np.multiply(dout, xVar), axis = 0)
  inv_sample_term = 1/(sample_var + eps)

  
  dx = gamma * (np.sqrt(inv_sample_term)/m)  * (m * dout  - (dout.sum(axis = 0)) - (((x-sample_mean) * inv_sample_term)) * (((x-sample_mean) * dout).sum(axis = 0)))
  #print dx.shape
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
