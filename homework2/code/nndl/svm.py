...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    num_train = X.shape[0]
    loss = 0.0
    a_mat = np.dot(X, np.transpose(self.W))
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    # ================================================================ #
    a_mat = np.dot(X, np.transpose(self.W))
    ayx = a_mat[np.arange(X.shape[0]), y]

    zj = 1+ a_mat - np.matrix(ayx).T
    zj[np.arange(X.shape[0]), y] -= 1
    ...
    # ================================================================ #
    ind_mat = np.zeros_like(zj)
    ind_mat[zj>0] = 1
    sum1 = np.sum(ind_mat, axis = 1)
    ind_mat[np.arange(X.shape[0]), y] = -sum1.T
    grad = np.matmul(ind_mat.T, X)
    grad = grad/X.shape[0]
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
